                ...
                <table className="spreadsheet-table">
                  <thead>
                    <tr>
                      ...
                      <th className="spreadsheet-head align-right">Empresas</th>
                      <th className="spreadsheet-head align-right">Fornecedores</th>
                    ...
                    {despesas.length ? (
                      despesas.map((item) => (
                        <tr key={`${item.sede}-${item.setor}-${item.despesa}`} className="spreadsheet-row">
                          ...
                          <td className="spreadsheet-cell align-right">{formatCurrency(item.total)}</td>
                          <td className="spreadsheet-cell align-right">{formatCurrency(item.totalEmpresa)}</td>
                          <td className="spreadsheet-cell align-right">{formatCurrency(item.totalFornecedor)}</td>
                          <td className="spreadsheet-cell align-center">{item.quantidade}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td className="spreadsheet-cell align-center" colSpan={5}>
                  ...
                  {despesas.length ? (
                    <tfoot>
                      <tr>
                        ...
                        <td className="spreadsheet-head align-right">{formatCurrency(sumBy(despesas, (item) => item.total))}</td>
                        <td className="spreadsheet-head align-right">{formatCurrency(sumBy(despesas, (item) => item.totalEmpresa))}</td>
                        <td className="spreadsheet-head align-right">{formatCurrency(sumBy(despesas, (item) => item.totalFornecedor))}</td>
