CREATE TABLE IF NOT EXISTS pagamentos (
    ...
    cod_vld VARCHAR(40) UNIQUE,
    perfil_criador VARCHAR(20),
    criado_por VARCHAR(80) NOT NULL,
    ult_editado_por VARCHAR(80),
    dt_sistema TIMESTAMP NOT NULL,
    dt_pagamento DATE NOT NULL,
    dt_vencimento DATE,
    dt_ult_edicao TIMESTAMP NOT NULL,
    status VARCHAR(20) NOT NULL DEFAULT 'LANCADO',
    sede VARCHAR(80) NOT NULL,
    sede_norm VARCHAR(80),
    colaborador VARCHAR(120),
    setor VARCHAR(80) NOT NULL,
    setor_norm VARCHAR(80),
    despesa VARCHAR(120) NOT NULL,
    despesa_norm VARCHAR(120),
    dotacao VARCHAR(120) NOT NULL,
    dotacao_norm VARCHAR(120),
    empresa_fornecedor VARCHAR(255),
    setor_pagamento VARCHAR(80),
    valor_total NUMERIC(12, 2) NOT NULL,
    descricao TEXT
);

CREATE TABLE IF NOT EXISTS pagamento_rateio (
    id BIGSERIAL PRIMARY KEY,
    pagamento_id BIGINT,
    nome VARCHAR(255) NOT NULL,
    valor NUMERIC(12, 2) NOT NULL,
    CONSTRAINT fk_pagamento_rateio_pagamento
...
CREATE INDEX IF NOT EXISTS idx_pagamentos_dt_pagamento ON pagamentos (dt_pagamento);
CREATE INDEX IF NOT EXISTS idx_pagamentos_dt_vencimento ON pagamentos (dt_vencimento);
CREATE INDEX IF NOT EXISTS idx_pagamentos_sede ON pagamentos (sede);
CREATE INDEX IF NOT EXISTS idx_pagamentos_setor ON pagamentos (setor);
CREATE INDEX IF NOT EXISTS idx_pagamentos_despesa ON pagamentos (despesa);
CREATE INDEX IF NOT EXISTS idx_pagamentos_dotacao ON pagamentos (dotacao);
CREATE INDEX IF NOT EXISTS idx_pagamentos_status ON pagamentos (status);
CREATE INDEX IF NOT EXISTS idx_pagamentos_setor_pagamento ON pagamentos (setor_pagamento);
CREATE INDEX IF NOT EXISTS idx_pagamentos_sede_norm ON pagamentos (sede_norm);
CREATE INDEX IF NOT EXISTS idx_pagamentos_setor_norm ON pagamentos (setor_norm);
CREATE INDEX IF NOT EXISTS idx_pagamentos_despesa_norm ON pagamentos (despesa_norm);
CREATE INDEX IF NOT EXISTS idx_pagamentos_dotacao_norm ON pagamentos (dotacao_norm);
CREATE INDEX IF NOT EXISTS idx_rateio_pagamento_id ON pagamento_rateio (pagamento_id);
CREATE INDEX IF NOT EXISTS idx_hist_pagamento_id ON pagamento_historico (pagamento_id);
CREATE INDEX IF NOT EXISTS idx_hist_dt_evento ON pagamento_historico (dt_evento);
